class AiController < ApplicationController
  ...
  def generate_message
    ...
    if api_key.nil? || api_key.empty?
      suggestions = (fallback_messages[tone] || fallback_messages['emotional']).map.with_index do |msg, idx|
        { id: "msg-#{idx}", message: msg, confidence: 0.9 }
      end
    ...
    begin
      ...
      if response.code == '200'
        ...
        if json_match
          ...
          suggestions = parsed['messages'].map.with_index do |msg, idx|
            { id: "msg-#{idx}", message: msg, confidence: 0.9 }
          end
          render json: { suggestions: suggestions }
        else
