...
const CardCreator: React.FC<CardCreatorProps> = ({
  ...
  // Base URL for the card view page - use dynamic window.location.origin
  const baseUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/card/view`
      : process.env.NEXT_PUBLIC_SITE_URL
        ? `${process.env.NEXT_PUBLIC_SITE_URL}/card/view`
        : "https://your-site.com/card/view";

  // Check if using fallback URL (for warning display)
  const isUsingFallbackUrl =
    baseUrl.includes("localhost") || baseUrl.includes("your-site.com");
  ...
  const handleToneSelect = async (tone: ToneType) => {
    ...
    // Use fallback messages directly (backend not available)
    try {
